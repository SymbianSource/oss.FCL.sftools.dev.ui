/*
* Copyright (c) 2006-2010 Nokia Corporation and/or its subsidiary(-ies). 
* All rights reserved.
* This component and the accompanying materials are made available
* under the terms of "Eclipse Public License v1.0"
* which accompanies this distribution, and is available
* at the URL "http://www.eclipse.org/legal/epl-v10.html".
*
* Initial Contributors:
* Nokia Corporation - initial contribution.
*
* Contributors:
*
* Description:
*
*/

package com.nokia.tools.theme.s60.ui.animation.controls;

import org.eclipse.jface.util.PropertyChangeEvent;

import com.nokia.tools.media.utils.layers.TimeSpan;
import com.nokia.tools.media.utils.layers.TimingModel;
import com.nokia.tools.media.utils.timeline.ITimeLine;

public interface ILayerEffectPropertySheetPage {

	TimingModel getTimeModel();

	TimeSpan getTimeSpan();

	ITimeLine getTimeLine();

	void propertyChange(PropertyChangeEvent event);

	void addTimeModelChangedListener(ITimeModelChangeListener listener);

	void removeTimeModelChangedListener(ITimeModelChangeListener listener);
}
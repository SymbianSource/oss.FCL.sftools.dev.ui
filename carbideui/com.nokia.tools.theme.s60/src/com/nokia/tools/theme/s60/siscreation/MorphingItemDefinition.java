...
public class MorphingItemDefinition extends BasicItemDefinition {
	...
	protected MorphingItemDefinition(Map properties) throws ThemeException {
		super(properties);
		this.animatableStatus = false;
	...
	private StringBuffer generateDefinitionForAnimatedSkin(SkinnableEntity sItem)
		...
		/*
		 * If there is no morphed data in the definition then the element must
		 * be written using 'Scaleable' item definition.
		 */
		if (itemGraphics.isMorphedGraphic() != true) {
			ScaleableItemDefinition scalObj = new ScaleableItemDefinition(
					properties);
			StringBuffer graphicsDef = scalObj.generateDefintion(sItem);
			errors.append(scalObj.getErrorString());
			return graphicsDef;
		}
		...
		for (int i = 0; i < layerList.size(); i++) {
			...
			for (int j = 0; layerEffectList != null
				...
				Map<Object, Object> nextEffectData = nextEffect.getAttributes();
	...
	private StringBuffer makeApplyEffectString(ImageLayer layerData,
		...
		Map<Object, Object> applyEffValues = layerData.getAttributes();
	...
	public static int getValueModelId(String valueModelref, Map valueModelMap) {

		Set set = valueModelMap.keySet();
		Iterator iter = set.iterator();
		while (iter.hasNext()) {
			Integer key = ((Integer) iter.next());
			BaseValueModelInterface vModel = (BaseValueModelInterface) valueModelMap
					.get(key);
			if (((String) vModel.getParameters().get(ThemeTag.ELEMENT_SEQNO))
					.equalsIgnoreCase(valueModelref))
				return key.intValue();
		}
		return 0;
	}

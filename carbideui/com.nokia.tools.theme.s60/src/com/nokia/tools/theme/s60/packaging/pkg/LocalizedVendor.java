...
/**
 * This class represents the localized vendor statement.<br/><br/><a
...
public class LocalizedVendor {
	...
	/**
	 * Constructs a localized vendor statement.
	 * 
	 * @param name name of the vendor.
	 */
	public LocalizedVendor(String name) {
		this.name = name;
	}
	 ...
	 */
	public static LocalizedVendor parse(String line) {
		if (line.startsWith("%")) {
			try {
				Object[] values = FORMAT.parse(line);
				String name = ((String) values[0]).trim();
				return new LocalizedVendor(name);
			} catch (Exception e) {
			}
		}
		return null;
